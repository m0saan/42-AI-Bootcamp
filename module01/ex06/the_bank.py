...
class Bank(object):
    ...
    def transfer(self, origin: Account, dest: Account, amount: float) -> bool:
        ...
        if not self.is_corrupted(origin) and origin in self.account and dest in self.account:
            index_origin = self.account.index(origin)
            index_dest = self.account.index(dest)
            if self.account[index_origin].value >= amount:
    ...
    @staticmethod
    def is_corrupted(origin):
        ok = False
        count = 0
        attributes = list(filter(lambda att: not att.startswith("__"), dir(origin)))
